...
#        self.c = config
    ...
    def add_training_vars(self):
        ...
        with tf.name_scope('peer_1_private_key_layer'):
            self.peer_1_private_weight = self.lw.get_weights('peer_1_private_key_weight', [self.c.plain_text_length/2, self.c.plain_text_length], 'xavier')
        ...
        with tf.name_scope('peer_2_fc_layer'):
            ...
            self.peer_2_fc_bias = self.lw.get_weights('peer_2_fc_bias', [self.c.plain_text_length],\
									initializer='constant', constant=0.1)
        ...
        with tf.name_scope('peer_1_conv1_layer'):
            ...
            self.peer_2_conv1_bias = self.lw.get_weights('peer_2_conv1_bias', [self.c.plain_text_length, 2], 'constant', 0.1)
        ...
        with tf.name_scope('peer_2_conv2_layer'):
            ...
            self.peer_2_conv2_bias = self.lw.get_weights('peer_2_conv2_bias', [self.c.plain_text_length/2, 4], 'constant', 0.1)
        ...
        with tf.name_scope('peer_2_conv3_layer'):
            ...
            self.peer_2_conv3_bias = self.lw.get_weights('peer_2_conv3_bias', [self.c.plain_text_length/2, 4], 'constant', 0.1)
        ...
        with tf.name_scope('peer_2_conv4_layer'):
            ...
            self.peer_2_conv4_bias = self.lw.get_weights('peer_2_conv4_bias', [self.c.plain_text_length/2, 1], 'constant', 0.1)
        ...
        with tf.name_scope('peer_2_private_key_layer'):
            self.peer_2_private_weight = self.lw.get_weights('peer_2_private_key_weight', [self.c.plain_text_length/2,\
                                                            self.c.plain_text_length], 'xavier')
        ...
        with tf.name_scope('input_training'):
            with tf.name_scope('peer_1'):
                ...
                with tf.name_scope('peer_1_key_op'):
                    ...
                    self.peer_1_out = tf.nn.sigmoid(self.peer_1_fc_key_out)
                    self.add_to_summaries([self.peer_1_out])
            #### ---------- peer 1 output = self.peer_1_out.

            with tf.name_scope('peer_2'):
                ## peer 2 key op.
                with tf.name_scope('peer_2_key_op'):
                    self.peer_2_fc_key_out = self.lw.FC_layer(self.peer_1_out, self.peer_2_private_weight,\
                                                        self.peer_2_private_bias)
                    self.peer_2_out = tf.nn.sigmoid(self.peer_2_fc_key_out)
                    self.add_to_summaries([self.peer_2_out])
                    self.peer_2_out = tf.expand_dims(self.peer_2_out, axis=2)
                with tf.name_scope('peer_2_conv1_layer'):
                    self.peer_2_conv1_out = self.lw.conv1d_layer(self.peer_2_out, self.peer_2_conv1_weight,\
                ...
                with tf.name_scope('peer_2_fc_layer'):
                    self.peer_2_fc_out = self.lw.FC_layer(self.peer_2_conv4_out, self.peer_2_fc_weight, \
                                                        self.peer_2_fc_bias)
                    self.peer_2_fc_out = tf.nn.sigmoid(self.peer_2_fc_out)
                    self.add_to_summaries([self.peer_2_fc_out])
                ## final result peer 2 = self.peer_2_fc_out.


    def add_loss_op(self):
        print("Peer 2 fc out shape")
        print(tf.shape(self.peer_2_fc_out))
        print("Input Placeholder shape")
        print(tf.shape(self.input_message_placeholder))
        abss = tf.abs(tf.subtract(self.peer_2_fc_out, self.input_message_placeholder))
